...
case class XY(x: Int, y: Int) {
  ...
  def signum = XY(x.signum, y.signum) // pointing direction
  ...
  def wrap(boardSize: XY) = { // espheric world
...
object XY {
  ...
  def rotateClockwise(direction45: Int): Int = {
    if (direction45 < 7) direction45 + 1
    else 0
  }

  def randomDirection: Int = {
    val rnd = new Random()
    rnd.nextInt(8)
  }
